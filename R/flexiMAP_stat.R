flexiMAP_stat <- function(UTRcounts_filt, InputTable, num_samples, covariates,
                         ...
                         type = c("ML", "BC", "BR"), link.phi=c("identity", "log", "sqrt"),name)
{
  ...
  #Initate output
  res_br  <- matrix(NA,nrow=nrow(short),ncol=ifelse(!missing(covariates),3+length(covariates),3))
  if(!missing(covariates)){
  ...
  for (i in 1:nrow(short)){
    ...
    if(length(ratio[ratio==0]) < num_samples){
      ...
      coef_br <- summary_br$coefficients[[1]][,4] ## Get the coefficients from the model
      res_br[i,2:ifelse(!missing(covariates),3+length(covariates),3)] <- coef_br
    }
