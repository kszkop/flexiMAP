flexiMAP_annotPrep <- function(polyAsite, reference, TINfilter)
{
  ...
  reference <- data.table::setDT(reference)[,list(transID = 'transID', chr=chr, strand=strand, Tstart=Tstart, Tend=Tend, CDSstart=CDSstart, UTR_start=UTR_start, test = ifelse(strand=='+', max(CDSend) <= UTR_start, min(CDSstart) >= UTR_start)), by = 'geneID']
